...
class Linear_QNet(nn.Module):
    ...
    def forward(self, x):   # take state as input (prediction function)
        x = F.relu(self.linear1(x))
        x = self.linear2(x)
        return x

    def save(self, file_name='dql_model.pth'): # save model to file
        model_folder_path = 'C:\CODING\PROJECTS\SNAKE_AI\models'
